...
import { FaExclamationCircle, FaBoxOpen, FaShippingFast, FaCheckCircle } from "react-icons/fa"; // אייקונים

const OrderManagement = ({ storeId }) => {
    const [orders, setOrders] = useState([]);
    const [sortOrder, setSortOrder] = useState("desc"); // מיון לפי תאריך
    const [sortStatus, setSortStatus] = useState(""); // מיון לפי סטטוס
    const { showAlert } = useAlert();

    useEffect(() => {
        axios.get(`http://localhost:5000/Transactions?store=${storeId}`)
            .then((res) => {
    ...
    const updateDeliveryStatus = (orderId, newStatus) => {
        axios
            .put(`http://localhost:5000/Transactions/${orderId}/updateDeliveryStatus`, { status: newStatus })
            .then(() => {
                setOrders(prevOrders =>
                    prevOrders.map(order =>
                        order._id === orderId
                            ? { ...order, delivery: { ...order.delivery, deliveryStatus: newStatus } }
                            : order
                    )
                );
    ...
    const updateTransactionStatus = (orderId, newStatus) => {
        axios
            .put(`http://localhost:5000/Transactions/${orderId}/updateTransactionStatus`, { status: newStatus })
            .then(() => {
                setOrders(prevOrders =>
                    prevOrders.map(order =>
                        order._id === orderId
    ...
    return (
        <div className="container mx-auto p-5">
            ...
            <div className="flex gap-4 mb-4">
                <button className="bg-primaryColor text-white px-4 py-2 rounded-lg shadow-md" onClick={toggleSortOrder}>
                    מיין לפי תאריך {sortOrder === "asc" ? "⬆" : "⬇"}
                </button>
    
                <select className="border px-3 py-2 rounded-lg shadow-md" onChange={handleSortStatusChange} value={sortStatus}>
            ...
            <div className="overflow-x-auto overflow-y-auto max-h-screen">
                <table className="min-w-full border-collapse border border-gray-300 shadow-lg rounded-lg overflow-hidden">
                    ...
                    <tbody>
                        ...
                        .map((order, index) => (
                            <tr key={order._id} className={`border-b ${index % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
                                <td className="border p-3 text-center">
                                    {new Date(order.createdAt).toLocaleDateString()}
                                </td>
                                <td className="border p-3 text-center">{order.transactionId.slice(-6)}</td>
                                ...
                                <td className="border p-3 font-bold text-center">
                                    <select
                                        onChange={(e) => updateTransactionStatus(order._id, e.target.value)}
                                        value={order.status}
                                ...
                                <td className="border p-3">
                                    ...
                                    <p><strong>ת.משוער:</strong> {order.delivery.estimatedDelivery ? new Date(order.delivery.estimatedDelivery).toLocaleDateString() : "לא זמין"}</p>
                                    <p><strong>מספר מעקב:</strong> {order.delivery.trackingNumber || "לא זמין"}</p>
                                    <select
                                ...
                                <td className="border p-3">
                                    {order.products.map((product) => (
                                        <div key={product.productId} className="border-b p-2">
                                            <p className="font-semibold">{product.name} - {product.quantity} יח'</p>
                                        </div>
                                    ))}
                                </td>
                            </tr>
