...
const ProductManagement = ({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("findProductByName", handleFindProductByName);
  }, [products]);
  ...
  useEffect(() => {
    const handleAutofillEdit = (e) => {
      // const productId = findProductIdByName(e.detail.productName);
      // console.log("🔍 מחפש מוצר לפי שם:", e.detail.productName, "נמצא ID:", productId);
      const updates = e.detail.newFields;
      // console.log("🔄 קיבלתי עדכון אוטומטי לעריכת מוצר:", updates);
      setPendingEditFields(e.detail);
      if (!updates) return;
  ...
  useEffect(() => {
    if (pendingEditFields && isEditing) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("openEditProduct", handlerEdit);
    return () => window.removeEventListener("openEditProduct", handlerEdit);
  }, []);
  ...
  useEffect(() => {
    if (pendingEditRequest && products.length > 0) {
      ...
      if (productId) {
        ...
        setEditingProduct(products.find((p) => p._id === productId));
        window.dispatchEvent(
          new CustomEvent("openEditProductForm", { detail: { productId } })
        );
  ...
  const findProductIdByName = (productName) => {
    ...
    console.log("🔍 מחפש מוצר לפי שם:", productName);
  ...
  const mapUpdatesToNewProduct = (updates) => {
    ...
    console.log("Mapped updates to new productppp:",editProductId);

    console.log("Mapped updates to new product:1", editingProduct);
    ...
    if (updates.manufacturingCost)
      mapped.manufacturingCost = updates.manufacturingCost;
    if (updates.allowBackorder !== undefined)
      mapped.allowBackorder = updates.allowBackorder;
    if (updates.internationalShipping !== undefined)
      mapped.internationalShipping = updates.internationalShipping;
    if (updates.images) mapped.images = updates.images;
    if (updates.discountPercentage)
      mapped.discountPercentage = updates.discountPercentage;
    ...
    if (updates.selectedCategories)
      mapped.selectedCategories = updates.selectedCategories;
    console.log("Mapped updates to new product:", mapped);
    return mapped;
  };
  ...
  return (
    ...
    {isAddingProduct && (
      ...
      lang="he-IL"
      onResult={(text) =>
        setNewProduct((prev) => ({ ...prev, nameHe: text }))
      }
      />
      </div>
      </div>
      ...
      lang="en-US"
      onResult={(text) =>
        setNewProduct((prev) => ({ ...prev, nameEn: text }))
      }
      />
      </div>
      </div>
      ...
      lang="he-IL"
      onResult={(text) =>
        setNewProduct((prev) => ({ ...prev, descriptionHe: text }))
      }
      />
      </div>
      </div>
      ...
      lang="en-US"
      onResult={(text) =>
        setNewProduct((prev) => ({ ...prev, descriptionEn: text }))
      }
      />
      </div>
      </div>
      <div className="mb-4">
      <label className="block font-bold mb-1 font-bold">
      ...
      בעת תמלול אמור "סיים", "פסיק" או "נקודה" כדי להפריד בין
      מאפיינים.
      </div>
      </div>
      ...
      <div className=" text-gray-400 text-xs mt-0 bg-opacity-80 px-1 rounded">
      .While dictating, say "comma", "dot", or "end" to separate
      features
      </div>
      </div>
      ...
      <div>
      <label className="block mb-1 font-bold font-bold">
      עלות ייצור
      </label>
      <input
