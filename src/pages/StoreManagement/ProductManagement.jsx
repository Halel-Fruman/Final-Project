...
import { Icon } from '@iconify/react'; // להבטיח שהמודול הזה מותקן
  ...
  const handleAddProduct = () => {
    ...
    axios
      .post("http://localhost:5000/products", {
        ...
        description: { en: newProduct.descriptionEn, he: newProduct.descriptionHe },
          ...
          <div className="bg-white p-8 rounded-lg shadow-lg w-1/2 max-w-lg max-h-[80vh]  overflow-y-auto ">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                onChange={(e) => setNewProduct({ ...newProduct, descriptionHe: e.target.value })}
              ></textarea>
            </div>
            ...
            <div className="flex justify-between mt-6">
              <button
                ...
                onClick={handleCancel} // סגירת המודאל כשיש לחיצה על ביטול
              >
                <Icon icon="material-symbols:cancel-outline-rounded" width="24" height="24" />
              </button>
              <button
                className="bg-green-500 text-white px-4 py-2 rounded-md shadow-md hover:bg-green-600"
                onClick={handleAddProduct} // שמירת המוצר
              >
      ...
      <ul className="mt-5 border-t">
        {products.map((product) => (
          <li key={product._id} className="flex justify-between items-center border-b py-2">
            <span>{product.name.en} / {product.name.he} - {product.price}₪</span>
          </li>
        ))}
      </ul>
    </div>
