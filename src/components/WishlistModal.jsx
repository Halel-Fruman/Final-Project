/**
 * WishlistModal.jsx
 * Modal that shows the user's wishlist. Supports global promo discounts.
 */
...
const WishlistModal = ({
  ...
  const promo = useGlobalPromo(); // global site-wide promo
  const [products, setProducts] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  /* load product objects for wishlist items */
  useEffect(() => {
    const load = async () => {
      setIsLoading(true);
      if (!wishlist?.length) {
        ...
        {isLoading ? (
          ...
          <ul className="space-y-4">
            {products.map((product) => {
              const active = getActiveDiscount(product.discounts, promo);
              const isOnSale = !!active;

              const finalPrice = isOnSale
                ? active.type === "percent"
                  ? product.price * (1 - active.value / 100)
                  : product.price - active.value
                : product.price;

              const percent = isOnSale
                ? active.type === "percent"
                  ? active.value
                  : Math.round((active.value / product.price) * 100)
                : 0;
                  ...
                  <div className="flex-1">
                    ...
                    {isOnSale ? (
                      <div className="text-xs ">
                        ...
                        <span className="mx-1 line-through text-gray-500">
